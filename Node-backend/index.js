...
io.on('connection', function(socket) {
    ...
    socket.on('play', function(data) {
        ...
        //init enemies for first player to join game
        if(clients.length === 0)
        {
            numOfEnemies = data.enemySpawnPoints.length;
            enemies = [];
            data.enemySpawnPoints.forEach(function(enenmySpawnPoint) {
                var enemy = {
                    name: guid(),
                    position: enenmySpawnPoint.position,
                    rotation: enenmySpawnPoint.rotation,
                    health: 100
                };
                enemies.push(enemy);
            });
        ...
        //send enemies when player joins
        ...
        //give confirmation to player upon joining
        console.log(currentPlayer.name+' emit: play '+JSON.stringify(currentPlayer));
        socket.emit('play', currentPlayer);
        //inform others the player has joined
    ...
    socket.on('health', function(data) {
        console.log(currentPlayer.name+' recv: shoot: '+JSON.stringify(data));
        //only change the health once, can be done by checking the originating player
